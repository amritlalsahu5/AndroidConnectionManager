...
public  class BasicHttpClient {
	 ...
	 */
	public BasicHttpClient( BasicRequestHandler requestHandler) {
	 ...
	 * @throws IOException 
	 */
	public HttpResponse execute(HttpRequest httpRequest)throws  IOException,HttpRequestException  {
		
		HttpResponse httpResponse = null;
		prepareConnection(httpRequest.getPath(),httpRequest.getHttpMethod(), httpRequest.getContentType(),httpRequest.getRequestHeaders());
	 ...
	 * @throws RequestTooLongException 
	 ...
	 * @throws ScoketTimeoutException 
	 */
	@SuppressWarnings("finally")
	public void prepareConnection(String path, HttpMethod httpMethod, String contentType, Map<String, String> req) throws HttpRequestException, IOException {
			uc = createSSLChannel(path);
			prepareConnection(uc, httpMethod, contentType);
			appendRequestHeaders(uc,req);
	}
...
//			uc.setChunkedStreamingMode(AppConstants.UPLOAD_CHUNK_SIZE);
		...
		if (uc != null) {
			try {
				...
				if(map != null){
					...
					if(col != null){
						Log.d(BasicHttpClient.class.toString(),
								"getRequestProperties:"
										+ Arrays.toString((col.toArray())));	
					}
				}

				Log.d(BasicHttpClient.class.toString(), "getConnectTimeout:"
						+ uc.getConnectTimeout());
				Log.d(BasicHttpClient.class.toString(), "getContentEncoding:"
						+ uc.getContentEncoding());
				Log.d(BasicHttpClient.class.toString(),
						"getContentType:" + uc.getContentType());
				Log.d(BasicHttpClient.class.toString(), "getContentLength:"
						+ uc.getContentLength());
				Log.d(BasicHttpClient.class.toString(), "getRequestMethod:"
						+ uc.getRequestMethod());
				Log.d(BasicHttpClient.class.toString(),
						"getDoInput:" + uc.getDoInput());
				Log.d(BasicHttpClient.class.toString(),
						"getDoOutput:" + uc.getDoOutput());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	protected void appendRequestHeaders(HttpURLConnection urlConnection, Map<String, String> req) {
		for (String name : req.keySet()) {
			...
			Log.d(BasicHttpClient.class.toString(), name + " : " + value);
		}
	}
